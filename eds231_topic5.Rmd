...
```{r pdf_import}
#setwd('dat/')
files <- list.files(path = here("dat"), pattern = "*EPA", full.names = TRUE)
ej_reports <- lapply(files, pdf_text)
ej_pdf <- readtext(file = here("dat", "*.pdf"),
...
```{r}
#followed the tutorial linked above
word <- c("water")

window_inside <- tokens_keep(toks1, 
                         pattern = word, 
                         window = 10) %>% 
  tokens_remove(pattern = word) %>% 
  tokens_tolower() %>% 
  tokens_remove(pattern = (stop_vec))

dfm_inside <- dfm(window_inside)

window_outside <- tokens_remove(toks1, 
                            pattern = word, 
                            window = 10) %>% 
  tokens_tolower() %>% 
  tokens_remove(patter = (stop_vec))

dfm_outside <- dfm(window_outside)

dfms <- rbind(dfm_inside, dfm_outside)

keyness_inside <- textstat_keyness(dfms,
                               target = seq_len(ndoc(dfm_inside)))
keyness_inside[1:10] %>% #just getting teh first ten
  kable()
```