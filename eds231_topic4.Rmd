...
# type = specific words
# tokens = total words (multiple of same word/type >= # types)
```
...
# column of lists bound together by their presence in a particular text
...
# `hunspell` package to process typos and misspelled words
```


```{r explore_hashtags}
hash_tweets <- tokens(corpus, remove_punct = TRUE) %>% 
               tokens_keep(pattern = "#*") #capture all hashtags

dfm_hash <- dfm(hash_tweets) #document feature matrix is an advanced text opject = matrix shows the location of each of the words in corpus

tstat_freq <- textstat_frequency(dfm_hash, n = 100) #non-tidy object into tidy format
head(tstat_freq, 10) 
...
topfeatures(dfm, 12) #most frequent words

#shortcut function for summary understanding of text
